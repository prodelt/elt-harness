...
1. **Нетривиальная задача любого рода →** `/pipeline`. Он сам классифицирует
   (TRIVIAL/MEDIUM/BUG/ARCH/COMPLEX/RESEARCH), обновит состояние проекта и выберет маршрут.
2. **Тривиальное** (опечатка, одна правка, вопрос) → просто делай, без церемоний.
3. **Закрытие любой задачи** — только с доказательством (вывод build/test/lint), не «готово».
...
| Фича / баг / рефактор в коде | **`/pipeline`** | он выберет `tdd` / `diagnose` / `architect-first` / `sprint` |
...
- **Дробить крупное** — COMPLEX-задачу не гнать в одном растущем контексте: слайсы через
  ...
  Главный рычаг экономии (см. `/elt-code` Шаг 3.5).
