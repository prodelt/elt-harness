...
- [ ] **T001** `elt spec approve [--spec specs/NNN-slug]` + `elt spec status`: пишет `specs/NNN-slug/approval.json` {approvedAt, specHash, tasksHash} (sha256 содержимого); повторный approve идемпотентен; правка spec.md/tasks.md после подписи → status=stale. Тест `tools/elt-spec-approve.test.js`. [files: ~/.claude/bin/elt.js, tools/elt-spec-approve.test.js]
