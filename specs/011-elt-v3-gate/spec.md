...
- **AC12.** Живой блок в чужом проекте (`<another-project> фузи музи`) новым гейтом, без
  ...
  Проект приёмки сменён с `<another-project>` решением 01.08: приёмка ведётся там же, где включается
...
- **AC14.** `red-proof:green` и повторный `grounding:no-reasons` дают `inconclusive`
  (коммит + строка в review-queue), НЕ `block`. `phantom-file`/`unreviewed-file`
  остаются `block`. Тест на каждый исход.
- **AC15.** `elt gate --full` игнорирует `oracleSelect`; fleet зовёт полный сьют
  перед merge; счётчик слайсов с последнего полного прогона растёт и сбрасывается.
- **AC16.** `elt stats` считает block-rate, прогоны судьи/коммит, оракул p50/p90,
  долю `l0-clean`/`inconclusive` и разложение блоков по источнику — из run-log.
- **AC17.** L0 несёт триггеры `out-of-scope` (файл вне `[files:]`) и `high-fanin`
  (число зависимых выше порога); FPR первичного судьи измерен на judge-bench.

Фазы L (T026/T027) и M (T028) критериев в spec.md не получают намеренно: их проверяемость
целиком в «Проверяемо:» слайсов, а формулировать AC на эволюцию до чисел T022/T023 —
значит фиксировать порог, который ещё не на чем калибровать.
