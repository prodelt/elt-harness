...
- [X] **T003** [live] live-fire каждого провайдера: scratch-git, тривиальная правка файла через claude -p / codex exec --sandbox workspace-write / agy -p; зафиксировано в providers.js: claude/codex — промпт через stdin (без изменений); agy — stdin ИГНОРИРУЕТСЯ как канал промпта (CLI отвечает про свои же argv-флаги), промпт идёт значением `-p` в argv; agy также игнорирует cwd процесса без явного `--add-dir` (иначе пишет в фикс. `~/.gemini/antigravity-cli/scratch`); agy резолвится в реальный `.exe` (не `.cmd`-шим) → спавнится без shell (иначе argv-промпт открыл бы shell-injection); agy-таймаут = exit 1 + "Error: timeout waiting for response" (не hang); реального 429 живьём не словили (нужен настоящий лимит, не спровоцировать намеренно)
