...
## Reference Libraries (vendored + 2026 methodology)

Deep references live in `references/`. Read them on demand for the engagement type.

### 2026 methodology (web-first — read these in addition to SKILL.md)
- `references/api-top-10.md` — OWASP API Security Top 10 (2023): BOLA, BOPLA, BFLA, unrestricted flows, improper inventory.
- `references/llm-security.md` — OWASP LLM Top 10 (2025): prompt injection (direct+indirect), tool abuse, excessive agency, RAG poisoning.
- `references/modern-web-vectors.md` — 15 vectors beyond OWASP 2021: SSTI, XXE, HTTP smuggling, JWT alg confusion, proto pollution, mass assignment, race conditions, subdomain takeover, SSRF evasion, CSP bypass, CORS reflection, OAuth, cache poisoning, DOM clobbering.
- `references/recon-2026.md` — ProjectDiscovery stack (subfinder/httpx/katana/nuclei/ffuf/gau/naabu/tlsx) for Step 1 black-box.
- `references/supply-chain.md` — dep confusion, typosquatting, GH Actions injection, SBOM, SLSA, cosign.
- `references/evidence-protocol.md` — artifact layout, repro.sh, SARIF export, chain-of-exploit, post-fix verification.
- `references/fp-filtering.md` — 5-gate suspect → finding promotion protocol (reachability/taint/payload/impact/artifacts). MANDATORY before any finding.

### Vendored libraries
- `references/INDEX.md` — master index + MITRE ATT&CK → tool mapping.
- `references/kill-chain-workflow.md` — full-scope engagement workflow beyond web (AD, cloud, assumed breach).
- `references/external/RedTeam-Tools/README.md` — 150+ tools curated by MITRE tactic (A-poc).
- `references/external/RedTeaming_CheatSheet/` — CRTP/CRTE/CRTO-grade cheatsheet tree (0xJs): `infrastructure/`, `windows-ad/`, `defense-evasion/`, `cloud/`, `OSINT.md`, `cobalt-strike.md`, `hashcracking.md`, `coding/`.

**Web audit order:** Step 0 scope → `recon-2026.md` → SKILL.md OWASP + `api-top-10.md` + `modern-web-vectors.md` + `llm-security.md` (if AI) + `supply-chain.md` (if repo/CI) → `fp-filtering.md` (every finding) → `evidence-protocol.md` (report). For AD/cloud/post-compromise load `kill-chain-workflow.md`.
...
- Scope split: `/security-best-practices` = defensive-by-default coding + stack cheatsheets. `/red-team` = offensive scanners + exploit verification. Do not duplicate.

## External Tools Arsenal
...
## Repository-Specific Methodology

Install/smoke-test commands live in **External Tools Arsenal** above. This section captures the AI orchestration + workflow for each integrated repo.
...
- Map to MITRE Reconnaissance + Initial Access risk hypotheses
- Enrich with breach-intel references when provided in scope

### theHarvester + awesome-hacker-search-engines
Sources: `github.com/laramies/theHarvester`, `github.com/edoardottt/awesome-hacker-search-engines`

Dork workflow:
1. Build query set from awesome-hacker-search-engines by target type (github, docs, cloud, files, buckets)
2. Execute passive collection via theHarvester (expanded sources: `-b bing,duckduckgo,crtsh,anubis,rapiddns,otx,urlscan`)
3. Deduplicate domains/emails/hosts, save planned dorks in `recon/dorks.txt` for reproducibility
4. Score findings by data sensitivity; normalize to evidence table `source | artifact | confidence | follow-up`

### PayloadsAllTheThings (github.com/swisskyrepo/PayloadsAllTheThings)
Strict non-destructive validation payloads only.
- Select minimal safe payload set per vector
- Record request/response evidence and false-positive notes
- Required categories in report: SQLi, XSS, Command Injection, SSRF, NoSQL Injection, Prompt Injection

### bettercap (github.com/bettercap/bettercap)
Authorized internal/pentest scopes only. Passive-first: `bettercap -eval "net.probe on; net.recon on; net.show; events.stream off; quit"`.
- Parse output into host/service inventory
- Correlate with OWASP A05/A10 + MITRE Discovery/Lateral Movement
- Auto-prioritize hosts by exposed management ports

### nishang (github.com/samratashok/nishang)
ATT&CK simulation references for detection readiness in lab/authorized AD only.

Allowed:
- Defensive validation of monitoring + detection rules
- Purple-team simulation with explicit written authorization

Forbidden:
- Unauthorized persistence, credential abuse, lateral movement on real production

AI orchestration: convert selected simulation modules → detection test cases. Output: expected telemetry, detection query, containment playbook.

### Coverage checklist (must be 8/8)
...
- [x] 0xJs/RedTeaming_CheatSheet (vendored at `references/external/RedTeaming_CheatSheet/`)
- [x] A-poc/RedTeam-Tools (vendored at `references/external/RedTeam-Tools/`, 150+ tools indexed by MITRE)
...
- Nishang / bettercap: lab + written authorization only. Forbidden on production.
