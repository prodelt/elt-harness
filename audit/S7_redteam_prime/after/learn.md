...
## After Learning — Promote to Global Patterns

For instincts with confidence >=0.9 that are cross-project reusable (not project-specific):
1. Draft a standalone "playbook" document describing the pattern, problem, and recipe.
2. Save to `~/.claude/skills/learned/{slug}.md` (kebab-case slug from trigger).
3. Format: problem statement, why linear approaches fail, step-by-step recipe, example.
4. Skip promotion if playbook with same slug already exists.

This builds a library of reusable deep patterns available globally (not just per-project instincts).

## Cross-pollination
- Review saved instincts periodically and promote high-confidence ones to rules.md or agent templates
- Copy instinct files between projects via `~/.claude/projects/*/memory/` junction
