...
// === CHECK 5: File size (B03) — warn at 500 LOC, block at 1200 LOC ===
try {
  if (filePath && fs.existsSync(filePath)) {
    const loc = fs.readFileSync(filePath, 'utf8').split('\n').length;
    const { fileSizeWarnLoc = 500, fileSizeBlockLoc = 1200 } = cfg.editEnforcer || {};
    if (loc >= fileSizeBlockLoc) {
      denyReasons.push(
        'FILE SIZE: ' + path.basename(filePath) + ' is ' + loc + ' LOC (hard ceiling ' + fileSizeBlockLoc + '). ' +
        'Each Edit reloads full file into tool_result (~' + Math.round(loc * 20 / 1000) + 'K tokens). ' +
        'SPLIT the file first — invoke Skill(architect-first) to design the split, then edit. BLOCKED.'
      );
    } else if (loc >= fileSizeWarnLoc) {
      warnings.push(
        'FILE SIZE: ' + path.basename(filePath) + ' is ' + loc + ' LOC (>' + fileSizeWarnLoc + ' red flag). ' +
        'Each Edit burns ~' + Math.round(loc * 20 / 1000) + 'K tokens of tool_result. ' +
        'Consider splitting before non-trivial changes (hard block at ' + fileSizeBlockLoc + ').'
      );
    }
  }
} catch (_) {}
