param(
    [string]$Source = (Join-Path $PSScriptRoot "architect-first\SKILL.md"),
    [string[]]$Roots = @(
        "$HOME\.claude\skills",
        "$HOME\.codex\skills",
        "$HOME\.gemini\skills"
...
if (-not (Test-Path -LiteralPath $Source)) {
    throw "Source architect-first SKILL.md not found: $Source"
}
...
$summary = foreach ($root in $Roots) {
    ...
    $targetDir = Join-Path $root "architect-first"
