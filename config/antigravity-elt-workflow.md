...
Before any task action, open `~\.gemini\skills\elt\SKILL.md` directly with ReadFile and read it completely. Do not search for it and do not claim that ELT is loaded until that read succeeds. Treat the text after `/elt` as the user's goal and follow the skill, with this adapter:

- Antigravity is the writer/orchestrator. It must never review, fix, judge, or attest its own code.
- For a code slice, use the existing `<repo-root>\tools\elt-loop.ps1` driver with `-WriterProvider agy` and `-Slices 1` unless the user explicitly requests an autonomous multi-slice run.
