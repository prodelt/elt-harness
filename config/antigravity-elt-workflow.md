...
Before any task action, open `C:\Users\espad\.gemini\skills\elt\SKILL.md` directly with ReadFile and read it completely. Do not search for it and do not claim that ELT is loaded until that read succeeds. Treat the text after `/elt` as the user's goal and follow the skill, with this adapter:
