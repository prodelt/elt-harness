...
Working directory: `C:\Claude playground\Pipiline setupper` (Pipeline Setupper repo) + `C:\Users\user\.amos` (нове ядро)
...
Створити `C:\Users\user\.amos` як окреме git-репо з CLI-ядром:
...
### Sprint 1 Done When
- [x] `node C:\Users\user\.amos\bin\amos.js event session-start` зі stdin-JSON `cwd=D:/Ametrin projects/Law_assistant` → ≤500ms, stdout ≤2KB валідного hook-JSON (~100-130ms, 185 bytes)
