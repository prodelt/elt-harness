...
**Evidence:**
```bash
$ ls C:/Users/user/.claude/hooks/errors.log
No such file or directory
$ grep -l "require.*logger" C:/Users/user/.claude/hooks/*.js | head
# → Ни один хук не импортит logger
```

`C:\Users\user\.claude\hooks\lib\logger.js` (полноценная реализация с rotation, 42 строки) импортируется только в `hook-stats.js` (CLI) и тестах. **27 продакшн хуков тихо падают без трейсов.**
...
**Evidence:**
```bash
$ grep CLAUDE_AUTOCOMPACT C:/Users/user/.claude/settings.json
...
**Evidence:**
```
C:/Users/user/.claude/projects/D--Ametrin-projects-sudoviy-master-try-3
C:/Users/user/.claude/projects/d--Ametrin-projects-Izi-tracker  ← lowercase d
```
...
`~/.codex/hooks.json` ссылается на `C:\Users\user\.claude\hooks\*.js`. Если Claude hooks ломаются → Codex тоже. Нет isolation.
