...
- Имя: AMOS, CLI `amos`.
- Ядро: `C:\Users\user\.amos` — ОТДЕЛЬНОЕ git-репо (git init).
...
1. `node C:\Users\user\.amos\bin\amos.js event session-start` со stdin-JSON `cwd=D:/Ametrin projects/Law_assistant` → ответ <500ms, stdout ≤2KB валидного hook-JSON.
