...
- Tracks ~162 files at drive root: the `ui-ux-pro-max-skill` source (now mostly **deleted** on disk → shows as ` D`), the user's **global Claude config** `Users/espad/.claude/hooks/**` + `.claude/skills/**` (37 files), and **personal CV** `Claude playground/CV/**`.
...
- **A. `git subtree split` (built-in, no install):** split `Users/espad/.claude/` history from `C:\.git` into a new branch, then seed `~/.claude/.git` from it. Preserves per-file history of the 16 commits. Most faithful.
...
- **C. `git-filter-repo` extraction:** install `git-filter-repo`, extract `Users/espad/.claude/**` into a clean repo. Cleanest history rewrite, but adds a tooling dependency.
