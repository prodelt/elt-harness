...
```
  [ok           ] паритет клиентов — источник 5.0.0 c520c61ee8fe
      claude: ok (5.0.0 c520c61ee8fe) — C:\Users\user\.claude\skills\elt\SKILL.md
      codex:  ok (5.0.0 c520c61ee8fe) — C:\Users\user\.codex\skills\elt\SKILL.md
      gemini: ok (5.0.0 c520c61ee8fe) — C:\Users\user\.gemini\skills\elt\SKILL.md
```
...
### Метаданные GitHub — проставлено

```powershell
gh repo edit prodelt/elt-harness --description "..." `
  --add-topic claude-code --add-topic claude-code-plugin --add-topic ai-agents `
  --add-topic code-review --add-topic developer-tools --add-topic harness `
  --add-topic spec-driven-development
```

Проверено `gh repo view --json description,repositoryTopics`: description release-ready,
7 topics на месте.

### Очередь ревью — разобрана, а не подметена

На входе в релизный гейт очередь несла 24 строки. Разбор:

| строки | что с ними |
| --- | --- |
| T003 ×4, T004 ×2 (спека 021) | причины устранены В ЭТОЙ сессии и проверены живьём (клон без CRLF, оракул 108/108) → закрыты |
| T001, T002 (спека 021) | устранены задачей T007 и ремонтом T003; проверено по коду (`UNC`/`isDiskRoot` — 17 упоминаний в тесте, `fileCount` — 3; `verifyRunnerHash().ok === true`; обе broken-руки достижимы) → закрыты |
| T001,T007 (legacy, `b6cd3b4`) | причина НЕ была устранена: судья требовал регресс на `isRed()` в `elt-brief.js`, и его не существовало. Написан здесь же (`tools/elt-brief.test.js`, 2 теста, включая границу префикса) |

Последняя строка — единственная, которую команда `elt review close` закрыла вместе с
остальными, хотя её причина была жива. Это записано как есть, а не исправлено молча: дыра
закрыта регрессом в том же слайсе. Проверка границы префикса нашла и ошибку в моём же
утверждении — `background-verification-passed` НЕ попадает под `startsWith('background-verify')`
(расхождение на 17-м символе), и тест зафиксировал фактическое поведение, а не ожидаемое.

Строки спеки 020 (13 шт.) не закрывались: они относятся к прошлому плану, их причины в этой
сессии не проверялись, и закрывать чужие вердикты «чтобы было чисто» — ровно то, против чего
заведена очередь.

### Что осталось за пользователем

`git push` заблокирован защитным хуком машины (`block-dangerous-git.js`) и выполняется
человеком. Порядок финала, который НЕ выполняется автоматически:

1. `git push origin feature/elt-v5-one-hour`;
2. дождаться зелёной матрицы GitHub Actions на `windows-latest` и `ubuntu-latest`;
3. fast-forward `main` на этот коммит (расхождения нет: `main` отстаёт, а не разошлась);
4. переключить default branch на `main` — сейчас он стоит на `feature/judge-bench-parallel-oracle`.

Public visibility, tag и GitHub Release — отдельное решение пользователя, спекой вынесены за
автоматику как необратимые.
