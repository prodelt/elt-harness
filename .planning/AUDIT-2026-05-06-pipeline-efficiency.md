...
**Причина:** Граф построен в `C:\Claude playground\Pipiline setupper\graphify-out\graph.json` и покрывает весь Pipeline Setupper проект. Реальные хуки живут в `C:\Users\user\.claude\hooks\` — за пределами этого репо.  
...
node C:/Users/user/.claude/hooks/graphify-session-init.js
...
# cmd /c graphify update . (запустить из C:/Users/user/.claude/hooks/)
```
