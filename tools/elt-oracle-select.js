...
// Бумага самого гейта: `spec.md`/`tasks.md`/`approval.json`. Её переписывает `elt spec approve`
// ПЕРЕД каждым прогоном (отметка [X] двигает tasksHash), и ни один тест её не читает — проверено
// по всему `tools/**.test.js`. Без этого исключения `approval.json` попадал бы в дифф каждого
// слайса, считался бы «не разобрать» и уводил выборку в «гнать все» ВСЕГДА, т.е. слой мёртв.
...
// попадания и там, где путь написан иначе (`require('./fleet/gate')`, `'node tools/x.js'`).
function needlesFor(rel) {
  ...
  // и команды) и имя, упирающееся в закрывающую кавычку (`require('./fleet/gate')`).
