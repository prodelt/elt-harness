...
- **S60 AMOS Sprint 0+1 + M4 closure (2026-06-10)**: AMOS (Agent Mini-OS) — новая v4 архитектура, заменяющая 109 хуков единым CLI-ядром. Sprint 0: ветка `system-upgrade/amos-kernel` + тег `v3-legacy` + `.planning/AMOS-BASELINE.md` с замерами (Pipeline Setupper + Law_assistant). Sprint 1: `C:\Users\espad\.amos` (отдельный git-репо, ветка `amos/sprint1-kernel`, 10 коммитов): `bin/amos.js` (CLI: `event session-start|stop`, `status`, `report`, `doctor`, `version`; fail-soft, 2KB cap, `os.homedir()`), `lib/db.js` (node:sqlite, 4 таблицы: sessions/events_metrics/projects/handoffs), `tests/` (52/52 PASS), `~/.claude/bin/amos.cmd` (глобальная обёртка). KPI: session-start ~100-130ms (<500ms), stdout 185 bytes (<2KB). M4 Merge & Final Verification: 4 параллельные ветки агентов (`amos/sprint0-baseline`, `amos/sprint1-kernel`, `amos/sprint1-state`, `amos/sprint1-tests`) объединены в `amos/sprint1-kernel`; `amos/` копии в репо синхронизированы с реальным `~/.amos` (52/52). **Sprint 0+1 закрыты. Следующий спринт: Sprint 2 — handoff continuity + интеграция SessionStart/Stop хуков всех 3 клиентов** (`.planning/PROMPT-SPRINT-2-AMOS.md`).
