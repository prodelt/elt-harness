  ...
  try {
    const wired = {
      hooks: {
        ...
        Stop: [
          { hooks: [{ type: 'command', command: 'node "C:/Users/user/.amos/bin/amos.js" event stop' }] }
        ]
      }
    };
    const claudePath = writeFixtureConfig(tmpDir, 'claude-settings.json', wired);
...
test('doctor: reports WARN when Stop exists but amos event stop hook is missing', () => {
  ...
  try {
    const unwired = {
      hooks: {
        Stop: [
          { hooks: [{ type: 'command', command: 'node "C:/Users/user/.claude/hooks/stop-verification.js"' }] }
        ]
      }
    };
    const claudePath = writeFixtureConfig(tmpDir, 'claude-settings.json', unwired);
        ...
        PreToolUse: [
          {
            ...
            hooks: [{ type: 'command', command: 'node "C:/Users/user/.amos/bin/amos.js" event pre-tool' }]
          }
        ]
      }
    };
...
test('doctor: reports WARN when PreToolUse exists but amos event pre-tool hook is missing', () => {
  ...
  try {
    const unwired = {
      hooks: {
        PreToolUse: [
          { matcher: 'Bash', hooks: [{ type: 'command', command: 'node "C:/Users/user/.claude/hooks/secret-scanner.js"' }] }
        ]
      }
    };
    const claudePath = writeFixtureConfig(tmpDir, 'claude-settings.json', unwired);
