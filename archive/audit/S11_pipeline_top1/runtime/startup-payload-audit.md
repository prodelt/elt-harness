...
- drift sources: `C:\Users\espad\.claude\settings.json`, `C:\Users\espad\.claude.json`, `.claude/settings.local.json`
...
- Подтверждено: в `C:\Users\espad\.claude\settings.json` есть `25` global plugin keys, из них `11` truthy-enabled.
...
- Подтверждено: в `C:\Users\espad\.claude.json` есть `1` duplicate project group для `D:/Mammoth ERP system` с тремя разными case-вариантами:
...
5. Нормализовать `C:\Users\espad\.claude.json` и схлопнуть duplicate project keys в один canonical path/case.
