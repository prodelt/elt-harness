...
**Результат**:
- `C:/Users/user/.claude/templates/project-settings.json` создан с базовым набором `enabledPlugins`.
- Project-level `.claude/settings.json` обновлены для 8 текущих рабочих roots под `C:\Claude playground` и `D:\Ametrin projects`; `C:\Users\user` использован как global baseline, но не перезаписывался.
...
- `C:\Users\user\.claude\templates\project-settings.json` → template содержит официальный формат `plugin-name@marketplace-name`.
**Зависимости**: 01
...
- `node audit/S11_pipeline_top1/skills/success-criteria-check.js --write-standard --roots C:\Users\user\.claude\skills C:\Users\user\.codex\skills C:\Users\user\.gemini\skills audit\S11_pipeline_top1\skills` → `OK: success criteria`, `checked: 66`, `updated: 2` on final rerun.
**Зависимости**: 13
...
- `node audit/S11_pipeline_top1/skills/success-criteria-check.js --roots C:\Users\user\.claude\skills\learn C:\Users\user\.codex\skills\learn C:\Users\user\.gemini\skills\learn audit\S11_pipeline_top1\skills\learn` → `OK: success criteria`, `checked: 4`.
...
- `node audit/S11_pipeline_top1/skills/success-criteria-check.js --roots C:\Users\user\.claude\skills\architect-first C:\Users\user\.codex\skills\architect-first C:\Users\user\.gemini\skills\architect-first audit\S11_pipeline_top1\skills\architect-first` → `OK: success criteria`, `checked: 4`.
...
- `node ~/.claude/hooks/token-budget.js --date=2026-04-23 --days=7 --limit=7` → `C:\Users\user\.claude\budget-2026-04-23.md`.
**Зависимости**: 01, 09
...
### [ ] ЗАДАЧА 38 | 75m | MED | HIGH — Skill distiller + token budget digest
...
### [ ] ЗАДАЧА 39 | 90m | MED | HIGH — Skill ranker по relevance/trust/token efficiency
...
### [ ] ЗАДАЧА 40 | 120m | HIGH | HIGH — gstack bridge как optional council mode
...
### [ ] ЗАДАЧА 41 | 120m | HIGH | HIGH — Research Autopilot skill-pack
...
### [ ] ЗАДАЧА 42 | 90m | MED | CRITICAL — Global promotion + rollback manifest
**Цель**: approved skills ставятся глобально воспроизводимо и откатываются одной командой.
**Файлы**: `audit/S11_pipeline_top1/skills/skill-promote.ps1`, `skill-rollback.ps1`, `skill-promotion.test.js`.
**Шаги**:
- promote из quarantine в `~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills`;
- manifest: source hash, target paths, backup paths, verifier command, approval proof;
- rollback восстанавливает предыдущие files без git reset.
**Проверка**: fixture promote → verify → rollback возвращает byte-identical исходное состояние.
**Зависимости**: 37, 38
...
- подтверждены findings: `25` global plugin keys, `147` local allow rules, `1` duplicate project-key group в `C:\Users\user\.claude.json`;
...
**Шаги**:
- backup-first normalize `C:\Users\user\.claude.json`;
...
### [ ] W10-07 | 120m | HIGH — LightRAG Python setup + ingest script
**Цель**: local LightRAG setup + `rag-ingest.py` script (per-project).  
**Зависимости**: W10-03..W10-06 (все манифесты готовы)

### [ ] W10-08 | 90m | MED — 10-question comparison table (all 4 projects)
**Цель**: доказать что RAG отвечает лучше чем grep на docs-вопросы.  
**Зависимости**: W10-07

### [ ] W10-09 | 30m | LOW — Обновить global route policy
