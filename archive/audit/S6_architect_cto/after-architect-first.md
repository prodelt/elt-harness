...
**Scope vs cto-playbook:** this skill owns the *workflow* (when to pause, what to validate, how to decide). `cto-playbook` owns the *standards catalog* (DORA targets, stack defaults, code quality rules, doc templates). Don't duplicate CTO standards here — reference them. Use both together for COMPLEX tasks.
...
- **File size >500 LOC = red flag** — if target file already exceeds 500 lines, split it BEFORE Edit. Each Edit tool call reloads the full file into the tool_result (~2K tokens per 100 LOC). Plan a split, apply it, THEN do the substantive edit. Hard ceiling: 800 LOC.
